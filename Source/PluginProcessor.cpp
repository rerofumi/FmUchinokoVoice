    ...
    , update_parameter_flag(false)
{
    ...
    addParameter(prm_base_frequency = new juce::AudioParameterFloat("PrmBaseFreq", "BaseFrequency", 60.0f, 440.0f, 80.0f));
...
{
    float skewedProportionX = 1.0f - std::exp(std::log(1.0f - (float)freq / 2.0 / 512.0f) * 0.2f);
    float line_x = (float)juce::jlimit(0, (int)PitchFilter::spectrumNum, (int)(skewedProportionX * (float)PitchFilter::spectrumNum));
